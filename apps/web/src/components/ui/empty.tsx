...
const emptySizeClassName = {
  ...
  tall: "min-h-88 gap-6 p-6 md:p-12",
  spacious: "gap-6 px-6 py-16 md:px-12",
...
function EmptyContent({
  ...
  direction = "column",
  ...props
}: React.ComponentProps<"div"> & { direction?: "column" | "row" }) {
  return (
    <div
      className={cn(
        "flex w-full min-w-0 max-w-sm items-center text-balance text-sm",
        direction === "row" ? "flex-row flex-wrap justify-center gap-2" : "flex-col gap-4",
        className,
      )}
