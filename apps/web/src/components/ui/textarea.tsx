...
type TextareaProps = React.ComponentProps<"textarea"> & {
  ...
  /** `annotation` is the quiet comment box that sits inside a diff or review row. */
  variant?: "default" | "annotation";
  /** Cap the auto-growing field; past this the content scrolls. */
  maxHeight?: "none" | "md";
  unstyled?: boolean;
};
...
function Textarea({
  ...
  const unstyled = unstyledProp || variant === "annotation";
  return (
    <span
      className={
        cn(
          ...
          variant === "annotation" &&
            "relative inline-flex w-full rounded-md border border-border/50 bg-background/20 font-sans text-foreground transition-colors focus-within:border-border/70",
          className,
        ) || undefined
      }
      data-size={size}
        ...
        render={(defaultProps) => (
          <textarea
            className={cn(
              ...
              maxHeight === "md" && "max-h-64",
              variant === "annotation" &&
                "min-h-12 cursor-text px-2.5 py-1.5 font-sans text-xs leading-5 caret-foreground max-sm:min-h-12",
            )}
            data-slot="textarea"
