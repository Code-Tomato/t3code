...
this. A few variant usages still remain in `src` (settings rows/screens, `NewTaskDraftScreen`,
`worktree-setup-card`) and still leak across platforms; they need remediation — do not add more.
Behavior (not styling) divergence branches on `Platform.OS` at runtime. When the native UI
